...
// CursorClientPos returns the current cursor position in client coordinates.
...
// Returns raw physical client coordinates — the same space as WM_MOUSEMOVE lParam
// (getXLParam/getYLParam), so event.GlobalX/GlobalY and CursorClientPos are consistent.
