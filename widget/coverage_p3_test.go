...
func TestAlertFullCoverage(t *testing.T) {
	...
	a.SetCloseBtn(true)
	a.SetTheme(AlertThemeSuccess)
	p3lay(tree, a, 0, 0, 300, 40)
	a.Draw(p3buf())
	a.SetTheme(AlertThemeWarning)
	a.Draw(p3buf())
	a.SetTheme(AlertThemeError)
	a.Draw(p3buf())
	a.Close()
	if !called {
...
func TestBreadcrumbCoverage(t *testing.T) {
	...
	b.SetOptions([]BreadcrumbItem{{Content: "Home", Href: "/"}, {Content: "Products", Href: "/p"}, {Content: "Detail"}})
...
func TestCollapseDrawCoverage(t *testing.T) {
	...
	c.SetBordered(true)
	c.OnChange(func([]string) {})
	c.SetPanels([]CollapsePanel{{Value: "1", Header: "One"}, {Value: "2", Header: "Two"}})
	c.Toggle("1")
	c.Toggle("2")
	if !c.IsActive("1") {
...
func TestMenuCoverage(t *testing.T) {
	...
	m := NewMenu(tree, cfg)
	m.SetValue("home")
	m.OnChange(func(string) {})
	m.SetItems([]MenuItem{
		{Value: "home", Content: "Home"},
		{Value: "about", Content: "About", Children: []MenuItem{{Value: "team", Content: "Team"}}},
	})
	m.ToggleExpanded("about")
	m.SelectItem("home")
...
func TestPaginationCoverage(t *testing.T) {
	...
	_ = p.Total()
	_ = p.PageSize()
	p.OnChange(func(PaginationPageInfo) {})
	p.SetTotal(100)
	p.SetPageSize(10)
	p.SetCurrent(5)
	_ = p.TotalPages()
...
func TestTimelineCoverage(t *testing.T) {
	...
	_ = tl.Items()
	tl.SetItemHeight(60)
	tl.AddItem(TimelineItem{Label: "Created", DotColor: "default"})
	tl.AddItem(TimelineItem{Label: "In Progress", DotColor: "primary"})
	tl.AddItem(TimelineItem{Label: "Error", DotColor: "error"})
	tl.AddItem(TimelineItem{Label: "Warning", DotColor: "warning"})
